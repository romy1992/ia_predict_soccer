...
class TestBuildRefereeFeaturesDataset(unittest.TestCase):
    ...
    def test_different_referees_have_independent_histories(self):
        ...
        self.assertAlmostEqual(row_rossi_second["referee_avg_cards_prior"], 10.0)  # solo la partita 1 di Rossi

    def test_missing_referee_always_uses_default_prior(self):
        base_date = datetime(2025, 1, 1, tzinfo=timezone.utc)
        matches = [
    ...
    def test_empty_matches_returns_empty_frame_with_expected_columns(self):
        ...
        for col in ["id_fixture", "referee_avg_cards_prior", "referee_matches_officiated_prior", "referee_has_history"]:
...
class TestBuildCardsFrameFromRecords(unittest.TestCase):
    ...
    def test_referee_features_present_and_within_expected_range(self):
        ...
        for col in ["referee_avg_cards_prior", "referee_matches_officiated_prior", "referee_has_history"]:
            self.assertIn(col, frame.columns)
