...
class DashboardService:
    ...
    def get_day_matches(
        ...
        for match in self._fetch_matches():
            ...
            if search_text:
                ...
                haystack = f"{row['home']} {row['away']} {row['league']}".lower()
                if search_l not in haystack:
                    continue

            rows.append(row)

        rows.sort(key=lambda x: (x.get("time") or "", x.get("league") or "", x.get("home") or ""))
        total_rows = len(rows)
        if limit > 0:
            rows = rows[:limit]
    ...
    def get_live_matches(
        ...
        rows = data.rows[:limit] if limit > 0 else data.rows
        return {
            "date": target_date.isoformat(),
            "total": len(data.rows),
            "returned": len(rows),
            "model_markets": data.model_markets,
            "rows": rows,
        }
