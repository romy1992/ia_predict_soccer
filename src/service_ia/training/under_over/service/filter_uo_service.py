...
class FilterUOService:
    ...
    # -------------------- SINGLE EVENT METHODS WITH ODDS --------------------
            ...
            if len(statistics_match) == 2 or self.predict:
                ...
                if not self.predict:  # Se è per addestrare metto la y
                    ...
                    match value:
                        ...
                        case 2.5:
                            result = 1 if total >= 3 else 0
                        case 3.5:
                            result = 1 if total >= 4 else 0
    ...
    # -------------------- MULTILABELS METHODS WITH ODDS --------------------
    def get_match_multilabel_events(self, filters: dict = None) -> list:
        matches_dict = self.filter_dict_matches(filters)
        dataset = []
        for match in matches_dict:
            if (match['odds'] and len(match['odds'][0]) > 0 and
                    all(k for k in match['odds'][0].keys() if k in self.events and match['odds'][0][k] and len(
                        match['odds'][0][k]) > 0)):
                statistics_match = match['statistics']
                if len(statistics_match) == 2 or self.predict:
                    stat_home, stat_away = None, None
        ...
        return sorted(dataset, key=lambda x: x.get('date_match'))

    def split_dataset_multilabel(self, dataset, type_calculation='mean') -> pd.DataFrame:
        """
        Splitta il dataset in X e y
        :param dataset: dataset da splittare
        :param type_calculation: tipo di calcolo 'std' o 'mean'
        ...
        def generate_feature(element):
            ...
            def calculate_label(event):
                ...
                under_itm, over_itm = None, None
                if event_data:
                    def generate_event_data(ev):
                        return {k: float(v) for k, v in event_data.items() if
                                ev in k and 'stat' not in k and pd.notna(v)}

                    under_itm = generate_event_data('under')
                    over_itm = generate_event_data('over')
        ...
        for match in matches_dict:
            ...
            if (match['odds'] and len(match['odds'][0]) > 0 and
                ...
                if len(statistics_match) == 2 or self.predict:
                    ...
                    if self.list_stats:  # Se bisogna aggiungere le statistiche
                        stats = self.get_specific_stat(match=match,
                                                       exclude=[],
    ...
    # -------------------- MATCH SENZA ODDS --------------------
    def get_matches_only_mean_stat(self, multilabel=False, event=None, **kwargs) -> list:
        """
        Genera il dataset in base alle partite filtrate senza considerare le quote
        :param multilabel: se True genera y multilabel (under over 1.5,2.5,3.5) altrimenti y singola
        :param event: se specificato genera y singola per l'evento (under_over_1_5, under_over_2_5, under_over_3_5)
        :return: dataset con solo medie specifiche e y multilabel (under over 1.5,2.5,3.5)
        """
        filters = {
            'status': ['FT' if not self.predict else 'NS'],
            'mean_statistics': "not None",
            'statistics': "not None", }
        # 'odds': ">= 0"}
        matches_dict = [m for m in self.filter_dict_matches(filters) if len(m['odds']) == 0]
        dataset = []
        for match in matches_dict:
            mean_stat = match['mean_statistics']
            statistics_match = match['statistics']
            id_team_home = match['id_team_home']
            id_team_away = match['id_team_away']

            # Controllo che ci siano le statistiche medie e totali
            if len(mean_stat) == len(statistics_match) == 2:
                match_records = {'id_fixture': match['id_fixture'],
                                 'name_home': match['name_home'],
                                 'name_away': match['name_away'],
                                 'date_match': match['date_match']}
                ...
                if not self.predict:  # Se è per addestrare metto la y
                    stat_home = statistics_match[0] if statistics_match[0]['statistics_team_id'] == id_team_home else \
                        statistics_match[1]
                    stat_away = statistics_match[1] if statistics_match[1]['statistics_team_id'] == id_team_away else \
                        statistics_match[0]
                    total_goals = stat_home['score_ft'] + stat_away['score_ft']
                    result = None
                    if multilabel:
                        u_o_1_5 = 1 if total_goals >= 2 else 0
                        u_o_2_5 = 1 if total_goals >= 3 else 0
                        u_o_3_5 = 1 if total_goals >= 4 else 0
                        result = [u_o_1_5, u_o_2_5, u_o_3_5]
                    else:
                        value = 2.5 if event == 'under_over_2_5' else 1.5 if event == 'under_over_1_5' else 3.5
                        match value:
                            case 1.5:
                                result = 1 if total_goals >= 2 else 0
                            case 2.5:
                                result = 1 if total_goals >= 3 else 0
                            case 3.5:
                                result = 1 if total_goals >= 4 else 0

                    match_records.update({'y': result})

                    if self.list_stats:
                        stats = self.get_specific_stat(match=match, stat_home=stat_home, stat_away=stat_away)
